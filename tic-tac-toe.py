...
def check_win():
    ...
    for coordinate in win_coordinates:
        ...
        # тут важно сперва проверять Х, т.к. в игре с ИИ по сути делается два хода за раз. Приоритет у человека
...
turn = 1  # это номер хода, по нему определяем кто ходит
step = greeting()  # 1 - играет человек с человеком. 2 - человек с ИИ
if step == 2:  # если играем с ИИ, то создаем список допустимых ходов для него
...
while True:
    if turn % 2 == 1:
        ...
        x_, y_ = ask_cell()
        grid[x_][y_] = "X"
        if step == 2 and turn < 9:  # если играем с ИИ и есть свободные клетки, то ИИ сразу делает ход
    ...
    turn += step  # увеличиваем на 1 при игре с человеком и на 2 при игре с ИИ
